 ...
 *  !! 주의
 *  - 속성을 추가할 시에는  /commons/resource/css/validator.css 에도 항목을 추가해야 한다.(복사 & 붙여넣기)
 *  사용방법 :
 *   HTML -
		...
		validate : function(form) {
    		...
    		for(var i = 0; i <elements.length;i++) {
				...
				for(var j = 0; j < classes.length; j++) {
    				...
    				if (Validator[className]) { // 클래스명과 일치하는 메서드가 있으면...
						var message = Validator[className](el.value,el,param);

						if (message) { // 오류가 있으면 메시지를 반환
...
/**************************************************************
1. Font Size 조절
2. 작성자 : 2007.11.12 Append by KMS
3. 사용법
	GisaFont = new SetFontSize();			// 탭 함수 선언
	GisaFont.name = "GisaFont";				// 탭 함수 이름 선언
	GisaFont.scaleFont(1);						// 폰트 사이즈 조절(-1: 1감소,1: 1증가)
**************************************************************/
		...
		this.scaleFont = function(val){
			...
			if (val > 0){
				if (this.fontSize <= 18) {
					...
					lineHeight = Math.round(1.2*this.fontSize);
	...
	function SetFontSizeEng(){
		...
		this.scaleFont = function(val){
			var content, lineHeight;
			if(document.getElementById("content_container_eng")){
				content = document.getElementById("content_container_eng");
			}else{
				content = document.getElementById("content_container_eng");
			}
			if (val > 0){
